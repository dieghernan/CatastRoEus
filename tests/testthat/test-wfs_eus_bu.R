test_that("BBOX Check errors", {
  expect_error(catreus_wfs_get_buildings_bbox(x = "1234"))
  expect_error(catreus_wfs_get_buildings_bbox(c("1234", "a", "3", "4")))
  expect_error(catreus_wfs_get_buildings_bbox(c(1, 2, 3)))
  expect_error(catreus_wfs_get_buildings_bbox(c(1, 2, 3, 4)))
})
...
test_that("BBOX Check projections", {
  ...
  expect_error(catreus_wfs_get_buildings_bbox(c(42.331139, -3.721619, 42.346365, -3.685913),
    srs = 25829, count = 10
  ))

  expect_message(catreus_wfs_get_buildings_bbox(
  ...
  obj <- catreus_wfs_get_buildings_bbox(c(43.026899, -2.433164, 43.308497, -2.063099),
    srs = 25830, count = 10
  )
  ...
  bbox <- get_sf_from_bbox(
    c(539226.596, 4744012.338, 539236.286, 4744133.782),
    srs = 25830
  )
  expect_s3_class(bbox, "sfc")

  obj2 <- catreus_wfs_get_buildings_bbox(bbox)
